...
- 「予測用の候補」の数、どれくらいまでいけそう??
  - =>5%サンプルで訓練したモデルに対して、ユニークユーザ当り20個はできそう。
    ...
    - 50個はダメだった
    - 30個ならOK！でも20個よりもスコアが下がってるから、これ以上増やす必要はないかも。
    - ＝＞以降はとりあえず、30個の中で、より良質な「候補」作りを目指す。
# 2022/04/25
特徴量生成における参考Doc
...
# 2022/05/05
- アイテムに関するラグ特徴量(lag, rolling, expanding)を作成した。
  - ＝＞ユーザに関するラグ特徴量も作る？
  - 参考
    - https://www.salesanalytics.co.jp/datascience/datascience070/
- ターゲットエンコーディングどうする？
  - そもそもImplicit Feedbackだから、単純に目的関数の平均を取る事はできない。
    - 対象期間の全トランザクションに占める、対象(アイテム、ユーザ)のトランザクションの割合とかにしてみる？
  - 時系列のデータだから、Orderd TEで行けそう。
- Negative Samplesの生成方法について
  - 参考(論文の2.2)
    - https://zheng-kai.com/paper/www_2022_chen_b.pdf
  - 1. Staticなサンプラー
    - 1. 一様分布に基づくサンプラー
      - (全ての項目が等しい確率で抽出される)
    - 2. 人気度分布に基づくサンプラー(人気のある項目はサンプリングされやすい)
      - 人気アイテムの方がユーザに晒されやすい。
      - ＝＞ユーザがアイテムに接触しているのにトランザクションが発生していない場合は、ユーザのそのアイテムへの嗜好度は低い事を意味する？？
  - 3. Adaptiveなサンプラー
  - **理想的なネガティブサンプル**とは？
    - ＝＞「**ユーザがアイテムと接触しても、選ばなかったと確信できるもの**」

## タスク
- ユーザのラグ特徴量を生成する○
- LGBMRankerクラスのメモリエラー対策(特徴量をマージするタイミング)を変える?
