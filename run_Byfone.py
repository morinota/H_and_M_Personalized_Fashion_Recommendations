...
VERSION = f"ByfoneModel_full{Config.use_full_sampling}_{Config.num_candidate_predict}Candidates"
...
def run_validation(val_week_id=104):
    ...
    model = ByfoneModel(transaction_train=train_df,
                                dataset=dataset,
...
def run_create_sub():
    ...
    if Config.use_full_sampling:
        ...
        dataset.read_data_sampled()
    # レコメンド結果を生成
    model = ByfoneModel(transaction_train=pd.DataFrame(),
                                dataset=dataset,
                                val_week_id=105,
