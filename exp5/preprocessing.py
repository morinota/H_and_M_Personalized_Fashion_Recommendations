...
class DataSet:
    ...
    def read_data(self):
        ...
        # データをDataFrame型で読み込み
        self.df = pd.read_csv(csv_train, dtype={'article_id': str}, parse_dates=[
            't_dat'])  # 実際の購買記録の情報
    ...
    def _drop_previous_month(self):
        ...
        import datetime
        # ２～１weeks ago
        mask = (df["t_dat"] >= datetime.datetime(2020,9,8)) & (df['t_dat'] < datetime.datetime(2020,9,16))
        self.train1 = df.loc[mask]
        # 3～2 weeks ago
        mask = (df["t_dat"] >= datetime.datetime(2020,9,1)) & (df['t_dat'] < datetime.datetime(2020,9,8))
        self.train2 = df.loc[mask]
        # 4～3 weeks ago
        mask = (df["t_dat"] >= datetime.datetime(2020,8,23)) & (df['t_dat'] < datetime.datetime(2020,9,1))
        self.train3 = df.loc[mask]
        # 5～4 weeks ago
        mask = (df["t_dat"] >= datetime.datetime(2020,8,16)) & (df['t_dat'] < datetime.datetime(2020,8,23))
