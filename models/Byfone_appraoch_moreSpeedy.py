...
class ByfoneModel:
    ...
    def _calculate_ldbw(self):
        ...
        self.transaction_train['dow'] = self.transaction_train['t_dat'].dt.dayofweek
        # 最終日は何曜日？？=>1=火曜日
        ...
        self.transaction_train['ldbw'] = (
            self.transaction_train['t_dat']
            - pd.TimedeltaIndex(data=self.transaction_train['dow'] - dow_last_ts, unit='D')
        )
        # t_datが水曜日以降のレコードの場合は、次の週(次の火曜日)としてldbwをカウントする
        self.transaction_train.loc[self.transaction_train['dow'] >= 2, 'ldbw'] = (
            self.transaction_train.loc[self.transaction_train['dow'] >= 2, 'ldbw']
